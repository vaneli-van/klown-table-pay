...
export function StudioEditor({ menuId, show, homeBase }: { menuId: string; show: (m: string) => void; homeBase: string }) {
  ...
  const { isLoading, error } = useQuery({
    ...
    queryFn: async () => {
      ...
      const c = await studioCatalogueList(t.menu.restaurant_id);
  ...
  const restaurantId = tree?.menu.restaurant_id ?? "";
  ...
  const refreshCatalogue = async () => { try { setCatalogue(await studioCatalogueList(tree?.menu.restaurant_id)); } catch { /* noop */ } };
  const importPos = async () => {
    ...
    try {
      ...
      const _t = await studioMenuGet(menuId);
      setTree(_t);
      setCatalogue(await studioCatalogueList(_t.menu.restaurant_id));
  ...
  return (
    <>
      <div className="st-toolbar">
        <a className="quiet" href={homeBase + (homeBase.startsWith("/admin") ? ("?restaurant=" + tree.menu.restaurant_id) : "")}>← Menus</a>
      ...
      {pageOpen && (
        <PageDialog tree={tree} restaurantId={restaurantId} show={show} onClose={() => setPageOpen(false)} onSaved={(t) => { if (t) setTree(t); }} run={run} />
      )}
      {shareOpen && (
        <ShareDialog tree={tree} show={show} onClose={() => setShareOpen(false)} />
      )}
    </>
  );
}
...
// ---------- Item dialog ----------
function ItemDialog({ state, currency, menuId, sections, restaurantId, show, onClose, onSaved, run }: {
  state: ItemDialogState; currency: string; menuId: string; sections: StudioSection[]; restaurantId: string; show: (m: string) => void; onClose: () => void;
  ...
  const save = async () => {
    ...
    if (state.catalogue) {
      await run(studioCatalogueUpsert(payload, restaurantId), { tree: false, ok: "Saved to food items" });
...
function PageDialog({ tree, restaurantId, show, onClose, onSaved, run }: {
  tree: StudioTree; restaurantId: string; show: (m: string) => void; onClose: () => void;
  onSaved: (tree: StudioTree | undefined) => void;
  run: <T>(p: Promise<T>, opts?: { tree?: boolean; ok?: string }) => Promise<T | undefined>;
}) {
  useEscape(onClose);
...
function ShareDialog({ tree, show, onClose }: { tree: StudioTree; show: (m: string) => void; onClose: () => void }) {
  useEscape(onClose);
